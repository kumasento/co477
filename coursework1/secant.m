function minimiser = secant(f, x0, x1, grad, varargin)
% SECANT quasi-newton method
    p = inputParser;
    p.addRequired('f', @(x) validateattributes(x, {'function_handle'}, ...
        {'scalar'}));
    p.addRequired('x0', @isscalar);
    p.addRequired('x1', @isscalar);
    p.addRequired('grad', @(x) validateattributes(x, ...
        {'function_handle'}, {'scalar'}));
    p.addOptional('bound', 0.0001, @isscalar);
    p.addOptional('maxIter', 100);
    p.parse(f, x0, x1, grad, varargin{:});
